...
%   model specification used is taken from TS 38.141-1 v15.2.0.
% 
...
%   of TS 38.141-1 and TS 38.141-2 used ('15.1.0', '15.2.0' or '15.7.0').
...
% 
%   % Display the Release 15 NR-TMs, defined for FR1.
%   
...
classdef hNRReferenceWaveformGenerator
    ...
    methods        
        ...
        %   The SV input is applicable to NR-TM only and specifies the version
        %   of TS 38.141-1 and TS 38.141-2 required ('15.1.0', '15.2.0' or
        %   '15.7.0').
        ...
        %   from the associated bandwidth configuration table (FR1 or FR2).    
            ...
            if contains(rc,["TM3.1" "TM3.1a"],'IgnoreCase',true)           % "NR-FR1-TM3.1", "NR-FR1-TM3.1a", "NR-FR2-TM3.1"
                targets = [0 2];    
            elseif contains(rc,["TM3.2", "TM3.3"],'IgnoreCase',true)      % "NR-FR1-TM3.2", "NR-FR1-TM3.3"
                targets = 1;
            elseif contains(rc,["TM1.2", "TM2", "TM2a"],'IgnoreCase',true) % "NR-FR1-TM1.2", "NR-FR1-TM2", "NR-FR1-TM2a", "NR-FR2-TM2"
                targets = 2;
            else
                targets = 0; 
            end 
        ...
        %   period of NUMSF subframes.
            ...
            % This requires k0 offset values which are contained in the resource grid SCS modulation information 
            figure;
...
function waveconfig = getTestModelPDSCHParameters(tdef,waveconfig,sv)
    ...
    % Using Rel 15 badged versions of 38.141: 15.0.0, 15.1.0, 15.2.0... 15.7.0...  
...
function pdsch = getPDSCHCommonParameters(fr,nrb,ncellid,mod,mcs)
    ...
    pdsch.RNTI = 0;                     % RNTI
    pdsch.NID = ncellid;                % Scrambling for PDSCH data part
    ...
    pdsch.PortSet = 0;                    % DM-RS ports to use for the layers
    ...
    pdsch.NIDNSCID = ncellid;             % PDSCH DM-RS scrambling identity (0...65535)
...
function waveconfig = getFRCPUSCHParameters(tdef,ncellid)
    ...
    pusch(1).NID = ncellid;                % Scrambling identity. It is in range 0 to 1023. Use empty ([]) to use physical layer cell identity                                                              
    ...
    pusch(1).NIDNSCID  = ncellid;          % Scrambling identity for CP-OFDM DMRS. It is in range 0 to 65535. Use empty ([]) to use physical layer cell identity NCellID                     
    pusch(1).NSCID = 0;                    % CP-OFDM DM-RS scrambling initialisation. It is either 0 or 1
    pusch(1).NRSID = ncellid;              % Scrambling identity for DFT-s-OFDM DM-RS. It is in range 0 to 1007. Use empty ([]) to use the physical layer cell identity NCellID                                                
    pusch(1).PowerDMRS = 0;                % Power boosting for DM-RS (in dB)
    ...
    pusch(1).PTRSNID = ncellid;            % Scrambling identity of PT-RS (0...1007). It is used only for DFT-s-OFDM                 
